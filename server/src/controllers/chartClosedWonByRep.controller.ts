...
// NOTE: no d3 needed in backend; we can do it with Maps.
...
export const getClosedWonByRep = (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // Count Closed Won (cleanStore.deals should already be 1 row per deal_id)
    ...
    return res.json({
      ...
      pie: {
        ...
        meta: {
          topN,
          totalClosedWon: rows.reduce((s, r) => s + r.value, 0),
        },
      },
    });
  } catch (err) {
